...
class Database:
    def __init__(self):
        ...
        self.dbCursor.execute(
            "CREATE TABLE IF NOT EXISTS patient_info (id PRIMARYKEY text, fName text, lName text, dob text, mob text, yob text, gender text, address text, phone text, email text, bloodGroup text, history text, doctor text)")
...
class LoginWindow:
    def __init__(self):
        ...
        self.window.configure(bg="#9c27b0")

        # Create labels for username and password
        Label(self.window, text="Username:").grid(row=0, column=0, padx=5, pady=5)
        Label(self.window, text="Password:").grid(row=1, column=0, padx=5, pady=5)
        ...
        Button(self.window, text="Login", command=self.login).grid(row=2, column=0, padx=5, pady=5)
        Button(self.window, text="Register", command=self.register).grid(row=2, column=1, padx=5, pady=5)
    ...
    def login(self):
        ...
        # Load the stored passwords from file.txt
        stored_passwords = self.load_passwords()

        # Check if the username exists and the password is correct
        if username in stored_passwords and self.verify_password(password, stored_passwords[username]):
    ...
    def register(self):
        ...
        if username in stored_passwords:
            ...
            # Store the username and hashed password in file.txt
            stored_passwords[username] = hashed_password
            self.save_passwords(stored_passwords)
    ...
    def save_passwords(self, stored_passwords):
        # Save the stored passwords to file.txt
        with open("file.txt", "w") as file:
            for username, hashed_password in stored_passwords.items():
...
class InsertWindow:
    def __init__(self):
        ...
        tkinter.Label(self.window, text="Patient Details", width=25).grid(pady=5, column=1, row=12)
        ...
        self.DetailsEntry.grid(pady=5, column=3, row=12)
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView["columns"] = ( "id", "fName", "lName", "dob", "mob", "yob", "gender", "address", "phone", "email", "bloodGroup", "history",
        "doctor")
